...
# ─────────────────────────────────────────
st.set_page_config(
    page_title="Imali — Your Financial Advisor",
    page_icon="💎",
    layout="centered",
    ...
    if st.session_state.auth_mode == "signup":
        ...
        <div style="text-align:center; padding: 2.5rem 1rem 1.5rem;">
            ...
            <h1 style="font-size:2.2rem;font-weight:800;color:#1A1F36;letter-spacing:-0.03em;margin:0">
                Create Your<br>Imali Account
            </h1>
            <p style="color:#6B7280;font-size:1rem;margin-top:0.8rem;font-weight:400;max-width:400px;margin-left:auto;margin-right:auto">
                Join thousands of users who trust Imali with their financial journey. Your data stays secure and private.
            </p>
        </div>
        """, unsafe_allow_html=True)
    else:
        ...
        with col2:
            if st.button("🚀 Create Account", key="signup_btn", use_container_width=True):
                if not all([name, email, password, confirm_password]):
                    ...
                    st.success(f"🎉 Welcome to Imali, {name}!")
                    st.rerun()
